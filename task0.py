# -*- coding: utf-8 -*-
"""
- Not requested in assignment used to find overall best solution
- Brute force solution of pricing of product 1 and pricing+promo matching of product 2
- Solution of a Less constrained version of the problem
    - Constraint of successful sale of product 1 before sale of product 2 not enforced
    - Promo assignment done in batches of 25 (each class can have multiple promo levels)
- All problem parameters known and used directly in solution
    - Conversion rates
...
for pc1 in range(N_PRICES):
    for pc2 in range(N_PRICES):
        for pc3 in range(N_PRICES):
            for pc4 in range(N_PRICES):
                ...
                for p in range(len(matching_axis_1)):
                    ...
                    cost_matrix[matching_axis_0 == 0, p] = conv_1[0, best_product_1_candidate[0]] * \
                        conv_2[0, pc1, promo_batch]*margin_2[pc1, promo_batch] * \
                        n_class[0]

                    cost_matrix[matching_axis_0 == 1, p] = conv_1[1, best_product_1_candidate[1]] * \
                        conv_2[1, pc2, promo_batch]*margin_2[pc2, promo_batch] * \
                        n_class[1]

                    cost_matrix[matching_axis_0 == 2, p] = conv_1[2, best_product_1_candidate[2]] * \
                        conv_2[2, pc3, promo_batch]*margin_2[pc3, promo_batch] * \
                        n_class[2]

                    cost_matrix[matching_axis_0 == 3, p] = conv_1[3, best_product_1_candidate[3]] * \
                        conv_2[3, pc4, promo_batch]*margin_2[pc4, promo_batch] * \
                        n_class[3]
        ...
        results = results.append({"class 1 price": price_2[pc1],
                                  ...
                                  "reward": reward}, ignore_index=True)  # append setting and reward to dataframe
